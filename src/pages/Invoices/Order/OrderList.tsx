...
const OrderList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const orderResponse = await fetch(`${API_BASE_URL}/order/listOrder`, {
        headers: {
      ...
      const vehicleResponse = await fetch(`${API_BASE_URL}/vehicle/listVehicle`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      const driverResponse = await fetch(`${API_BASE_URL}/auth/users`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteMultipleOrders = async () => {
    ...
    for (const id of selectedCheckBoxDelete) {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/order/deleteOrder/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEditOrder = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/order/updateOrder/${selectedOrder?._id}`, {
        method: "PUT",
        headers: {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        enableColumnFilter: false,
        cell: (cell: any) => {
          return <span>{handleValidDate(cell.getValue())}</span>;
        },
      },
      {
        header: "Pick Up Time",
